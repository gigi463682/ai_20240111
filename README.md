...
│ conv2d_6(共2D)│(沒有, 32, 32, 32)│        896 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ relu1(瑞露)│(沒有, 32, 32, 32) │          0 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ max_pooling2d (最大冷卻2D) │ (沒有, 16, 16, 32) │          0 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ conv2d_7 (共2D) │ (沒有, 16, 16, 64) │     18,496 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ relu2(瑞露)│(沒有, 16, 16, 64)│          0 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ max_pooling2d_1(最大冷卻2D)│(沒有, 8, 8, 64)│          0 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ conv2d_8(共2D)│(沒有, 8, 8, 64)│     36,928 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ relu3(瑞露)│(沒有, 8, 8, 64)│          0 │
...
│ shend_1(密度)│(沒有, 10)│        650 │

└ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

 >總共面額: 957,536 (3.65 MB)
 
 >火車: 319,178 (1.22 MB)
 
 >不可訓練的參數: 0 (0.00 B)
 
 >優化器參數: 638,358 (2.44 MB)
...
│ input_layer_4(輸入層)│(沒有, 32, 32, 3)│          0 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ inv_1(捲積)│ [(沒有, 32, 32, 3),│         26 │

│ │ (沒有, 32, 32, 9, 1, 1) ] │ │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ re_lu_4 (瑞露) │ (沒有, 32, 32, 3) │          0 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ max_pooling2d_2 (最大冷卻2D) │ (沒有, 16, 16, 3)│          0 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ inv_2(捲積)│ [(沒有, 16, 16, 3),│         26 │

│ │(沒有, 16, 16, 9, 1, 1)] │ │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ re_lu_6(瑞露)│(沒有, 16, 16, 3)│          0 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ max_pooling2d_3(最大冷卻2D)│(沒有, 8, 8, 3)│          0 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ inv_3(捲積)│ [(沒有, 8, 8, 3),(沒有,  │         26 │

│ │ 8, 8, 9, 1, 1)] │ │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ re_lu_8(瑞露)│(沒有, 8, 8, 3)│          0 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ flatten_1(展平)│(沒有, 192)│          0 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ shend_2(密度)│(沒有, 64)│     12,352 │

├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

│ shend_3(密度)│(沒有, 10)│        650 │

└ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── ─ ─ ─ ─

 >總共面額: 39,230 (153.25 KB)
 
 >火車: 13,074 (51.07 KB)
 
 >不可訓練的參數: 6 (24.00 B)
 
 >優化器參數: 26,150 (102.15 KB)
...
```
 ...
 1/1 ━━━━━━━━━━━━━━━━━━━━ 0s 9ms/step
```
![image](https://github.com/gigi463682/ai_20240111/blob/aee39e971a74668e9c482eac23c33c4b81b78aab/ai0111_png/involution_24_1.png)
---
__結論__
>在此示例中,主要重點是構建 Involution 哪一層 可以輕鬆重複使用。雖然我們的比較基於特定的 任務,請隨時將圖層用於不同的任務並報告您的 結果。

>在我看來,融合的關鍵在於它 與自我注意的關係。特定位置背後的直覺 通道語音處理在許多任務中都是有意義的。

>向前邁進:

>>看 揚尼克的視頻 繼續 進化以更好地理解。
>>實驗進積層的各種超參數。
>>使用捲積層構建不同的模型。
>>嘗試完全構建其他內核生成方法。
>您可以使用託管的訓練有素的模型 擁抱臉部集線器 並嘗試演示 擁抱臉部空間。
